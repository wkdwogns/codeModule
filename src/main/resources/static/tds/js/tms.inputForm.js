    ...
    * */
    var detailMapping = function(data, parentNode){
        ...
        obj.each(function(key, value){
            ...
            var tagNm = $value[0].nodeName.toUpperCase();

            if(tagNm == "INPUT" || tagNm == "TEXTAREA" || tagNm == "SELECT"){
                if($value.attr("type") == "checkbox"){
                    var sel = (tms.isNotEmpty($value.attr("id")))? "#"+$value.attr("id") : "[name="+$value.attr("name")+"]";
                    if(data[objNm] == $(sel).val()){
                        $(sel).prop("checked", true);
                    }
                } else if($value.attr("type") == "radio"){
                    $("input:radio[name="+$value.attr("name")+"]:input[value="+data[objNm]+"]").prop("checked", true);
                } else {
